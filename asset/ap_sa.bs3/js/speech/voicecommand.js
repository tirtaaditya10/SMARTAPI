...
(function(undefined) {"use strict";
	...
	//debug_state = false, // decleared in app.config.js
	//debug_style = 'font-weight: bold; color: #00f;', // decleared in app.config.js
	...
	root.smartSpeechRecognition = {
		...
		init : function(commands, resetCommands) {
			...
			recognition.onstart = function() {
				...
				//debug_state
				if (debug_state) {
					root.console.log('%c ✔ SUCCESS: User allowed access the microphone service to start ', debug_style__success);
					root.console.log('Language setting is set to: ' + recognition.lang, debug_style);
				}
			...
			recognition.onerror = function(event) {
				...
				switch (event.error) {
					...
					case 'service-not-allowed':
						...
						//debug_state
						if (debug_state) {
							root.console.log('%c WARNING: Microphone was not detected (either user denied access or it is not installed properly) ', debug_style__warning);
						}
			...
			recognition.onresult = function(event) {
				...
				for (var i = 0; i < results.length; i++) {
					...
					if (debug_state) {
						root.console.log('Speech recognized: %c' + commandText, debug_style);
					}
					...
					for (var j = 0, l = commandsList.length; j < l; j++) {
						...
						if (result) {
							...
							if (debug_state) {
								root.console.log('command matched: %c' + commandsList[j].originalPhrase, debug_style);
		...
		addCommands : function(commands) {
			...
			if (debug_state) {
				root.console.log('Commands successfully loaded: %c' + commandsList.length, debug_style);
			}
		},
