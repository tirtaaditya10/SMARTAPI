 ...
 */
	...
	breadcrumb = $('#ribbon ol.breadcrumb'),
/*
 * Top menu on/off
 */
	menu_top = false,
 ...
 * detected a class is inserted to $.root_ and the variable this_device
 ...
 */	
	init_app = (function(app) {
			...
			var smartActions = {
			    ...
			    userLogout: function($this){
					...
					$.SmartMessageBox({
						...
						content : $this.data('logout-msg') || "Thank you for your good work today",
						buttons : '[No][Yes]'
			   ...
			   // MINIFY MENU
			    ...
			    toggleShortcut: function(){
					...
					menu_shortcut.find('a').click(function(e) {
						e.preventDefault();
						window.location = $(this).attr('href');
						setTimeout(menu_shortcut_hide, 300);
				
					});
				
					// SHORTCUT buttons goes away if mouse is clicked outside of the area
					$(document).mouseup(function(e) {
						if (!menu_shortcut.is(e.target) && menu_shortcut.has(e.target).length === 0) {
							menu_shortcut_hide();
						}
					});
					
					// SHORTCUT ANIMATE HIDE
					function menu_shortcut_hide() {
						menu_shortcut.animate({
							height : "hide"
						}, 300, "easeOutCirc");
			...
			if (!menu_top) {
				if (!null) {
					$('nav ul').jarvismenu({
						...
						opened_sign : '<em class="fa fa-minus-square-o"></em>'
					});
				} else {
		...
		app.domReadyMisc = function() {
            ...
            $('#search_global').focusin(function() {
                $(this).attr('placeholder', 'Find document, reports, catalog and more')
            });
            $('#search_global').focusout(function() {
                $(this).attr('placeholder', 'Global Search')
            });
			...
			$('#activity').click(function(e) {
				...
				if (!$this.next('.ajax-right-dropdown').is(':visible')) {
					$this.next('.ajax-right-dropdown').fadeIn(150);
					$this.addClass('active');
				} else {
					$this.next('.ajax-right-dropdown').fadeOut(150);
					$this.removeClass('active');
				}
		
				var url  = '/crm/tpl/ajax/full/ajax/notify/notifications.html';
				
				container = $('.ajax-notifications');
				
				load_url(url, container);
				//clear memory reference
				$this   = null;
				url     = null;
				e.preventDefault();
			...
			$(document).mouseup(function(e) {
				if(!$('.ajax-right-dropdown').is(e.target) && $('.ajax-right-dropdown').has(e.target).length === 0) {
					$('.ajax-right-dropdown').fadeOut(150);
					$('.ajax-right-dropdown').prev().removeClass("active");
				}
			});
		 ...
		 * Description: fire with jQuery(document).ready...
		 */
		app.mobile_check_activation = function(){
			if ($(window).width() < 979) {
			...
			if (debug_state){
				console.log("mobile_check_activation");
			}
		}
		/* ~ END: MISCELANEOUS DOM */
		return app;
 ...
 */
	jQuery(document).ready(function() {
		init_app.SmartActions();
		init_app.leftNav();
		init_app.domReadyMisc();
	});
/*
...
*/
	$('#main').resize(function() {
		init_app.mobile_check_activation();
	});
		...
		jarvismenu : function(options) {
			...
			$this.find("li").each(function() {
				if ($(this).find("ul").length !== 0) {
					...
					$(this).find("a:first").append("<b class='collapse-sign'>" + opts.closed_sign + "</b>");
			...
			$this.find("li.active").each(function() {
				...
				$(this).parents("ul").parent("li").find("b:first").html(opts.opened_sign);
				...
				if ($(this).parent().find("ul").length !== 0) {
								...
								if (close) {
									if ($(this).parent().find("ul") != visible[visibleIndex]) {
										$(visible[visibleIndex]).slideUp(opts.speed, function() {
											$(this).parent("li").find("b:first").html(opts.closed_sign);
					...
					if ($(this).parent().find("ul:first").is(":visible") && !$(this).parent().find("ul:first").hasClass("active")) {
						$(this).parent().find("ul:first").slideUp(opts.speed, function() {
							$(this).parent("li").removeClass("open");
							$(this).parent("li").find("b:first").delay(opts.speed).html(opts.closed_sign);
						});
					}
					else {
						$(this).parent().find("ul:first").slideDown(opts.speed, function() {
							...
							$(this).parent("li").find("b:first").delay(opts.speed).html(opts.opened_sign);
						});
					} // end else
				} // end if
			});
 ...
 */
	...
	function loadScript(scriptName, callback) {
	   if (!js_array[scriptName]) {
	      ...
	      js_array[scriptName] = promise.promise();

	   }
	   else if (debug_state)
	      root.root.console.log("This script was already loaded %c: " + scriptName, debug_style__warning);

	   js_array[scriptName].then(function () {
	...
	if($.navAsAjax) {
	    ...
	    $(document).on('click', 'nav a[href!="#"]', function(e) {
			...
			if (!$this.parent().hasClass("active") && !$this.attr('target')) {
			    ...
			    // you could also do here:  this_device === "mobile" - and save a little more memory
 ...
 */
	function check_url() {
		...
		if (url) {
			...
			// debug_state
			if (debug_state){
				root.console.log("Page title: %c " + document.title, debug_style__green);
			}
			
			// parse url to jquery
			load_url(url + location.search, container);
		}
		else {
			...
			// update hash
			// window.location.hash = $this.attr('href');
			var $this = $('nav a[href!="#"]').eq(0);
			window.location.hash = $this.attr('href').replace($this.attr('baseURI', '#'));
 ...
 * LOAD AJAX PAGES
 */ 
	function load_url(url, container, callback) {
		// debug_state
		if (debug_state){
			root.root.console.log("Loading URL: %c" + url, debug_style);
		}
		if(AGV.xhr)
			AGV.xhr.abort();
		...
		AGV.xhr = $.ajax({
			...
			beforeSend : function() {
				...
				if ($.navAsAjax && $(".google_maps")[0] && (container[0] == $("#content")[0]) ) {
					...
					collection.each(function() {
					    ...
					    if(i == collection.length + 1) {
							...
							// debug_state
							if (debug_state){
								root.console.log("Destroying maps.........%c" + this.id, debug_style__warning);
							}
						}
					});
				...
				if ( $.navAsAjax && (container[0] == $("#content")[0]) ) {
					...
					// this function is below the page_function for all pages that has instances

					if (typeof page_destroy == 'function') {
					  try {
						  page_destroy();
 ...
 * UPDATE breadcrumb
 */ 
	function draw_breadcrumb(opt_breadcrumbs) {
		var a = $("nav li.active > a"),
			b = a.length;
	
		breadcrumb.empty(),
		breadcrumb.append($("<li>Home</li>")), a.each(function() {
			breadcrumb.append($("<li></li>").html($.trim($(this).clone().children(".badge").remove().end().text()))), --b || (document.title = breadcrumb.find("li:last-child").text())
		});
		
		// Push breadcrumb manually -> draw_breadcrumb(["Users", "John Doe"]);
		// Credits: Philip Whitt | philip.whitt@sbcglobal.net
		if (opt_breadcrumbs != undefined) {
			$.each(opt_breadcrumbs, function(index, value) {
				breadcrumb.append($("<li></li>").html(value));
				document.title = breadcrumb.find("li:last-child").text();
			});
		}
	}
