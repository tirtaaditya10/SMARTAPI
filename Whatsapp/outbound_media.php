...
function logErr($data){
  $logPath ="Logs/WA_Media_Log_".date('Y-m-d').".log";
...
function ReadDB($Token){
	...
	echo "<br/>get data";
	$sql = "EXEC SP_WA_TO_SEND_GetALL @TypeID=3";
	...
	while( $row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC) ) {	
		...
		if($row["Channel"]=="0"){
			...
			if(empty($param)){
				$res = sendWA($headers,$json3,3);
				$data = json_decode($res, true);
			}elseif(empty($param2)){
				$res = sendWA($headers,$json,3);
				$data = json_decode($res, true);
			}elseif(!empty($param5)){
				$res = sendWA($headers,$json4,3);
				$data = json_decode($res, true);
			}elseif(!empty($param6)){
				$res = sendWA($headers,$json6,3);
				$data = json_decode($res, true);
			}elseif(!empty($param3)){
				$res = sendWA($headers,$json5,3);
				$data = json_decode($res, true);
			}else {
				$res = sendWA($headers,$json2,3);
				$data = json_decode($res, true);
			}			
...
if($jam >= 4 && $jam <= 24)  {
echo "ok";
$my_file = '/web/sab-api/wa/login.conf';
