{if !isset($data)}
	{assign 'data' $sys.rsp.dat}
{/if}
{if !isset($pgP)}
	{assign 'pgP' $sys.nav.pgP}
{/if}
<div class="table-scrollable">
	<table class="footable table toggle-arrow-circle table-hover table-striped">
	    ...
	    <tr class="blue">
			...
			<th data-sort-ignore="true">Batch Sampling </th>
			<th data-sort-ignore="true">Is Redundant </th>
			<th data-sort-ignore="true">Is Double </th>
			<th data-sort-ignore="true">Created By </th>
			<th data-sort-ignore="true">Created On </th>
			{include file="{$sys.uix.elm}/tbl/th_hdr.tpl"}
	    </tr>
	    </thead>
	    <tbody>
	    {if $data}
	        {foreach from=$data item=i}
                <tr id="{$sys.req.rid}/{$i.id}">
                    <td class="align-center">{$i@iteration + ($pgP * $sys.req.l)}</td>
