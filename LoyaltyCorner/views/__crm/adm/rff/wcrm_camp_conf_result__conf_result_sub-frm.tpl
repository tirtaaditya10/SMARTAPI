<form id="F{$sys.req.rid}{$sys.req.pid}" method="post" action="#{$sys.req.rid}/{$sys.req.pid}" class="cForm form-horizontal" role="form">
    <fieldset>
        ...
        &ensp;
         {include file="{$sys.uix.elm}/frm/bs-hrz/elm_text.tpl"      label="CONF RESULT"  name="wcrm_camp_conf_result" required=true}
         {include file="{$sys.uix.elm}/frm/bs-hrz/elm_text.tpl"      label="SUB RESULT"  name="wcrm_camp_conf_result_sub" required=true}
