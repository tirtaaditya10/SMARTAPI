<form id="F{$sys.req.rid}{$sys.req.pid}" method="post" action="#{$sys.req.rid}/{$sys.req.pid}" class="cForm form-horizontal" role="form">
    <fieldset>
        ...
        {include file="{$sys.uix.elm}/frm/bs-hrz/elm_text.tpl" label="SUB SOURCE NAME" 	name="wcrm_loyalty_source_sub" required=true}
        {include file="{$sys.uix.elm}/frm/bs-hrz/elm_text.tpl" label="SOURCE ID" 	name="wcrm_loyalty_source_id"}
