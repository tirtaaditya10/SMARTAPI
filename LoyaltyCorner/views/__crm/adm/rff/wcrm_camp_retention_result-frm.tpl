<form id="F{$sys.req.rid}{$sys.req.pid}" method="post" action="#{$sys.req.rid}/{$sys.req.pid}" class="cForm form-horizontal" role="form">
    <fieldset>
        ...
        {include file="{$sys.uix.elm}/frm/bs-hrz/elm_text.tpl" label="Retention Result"  name="wcrm_camp_retention_result" required=true}
        {include file="{$sys.uix.elm}/frm/bs-hrz/elm_number.tpl" label="Order No"  name="seq" required=true}
