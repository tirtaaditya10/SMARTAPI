...
{/block}
{block name="th_ext"}
   {include file="{$sys.uix.elm}/tbl/th_col.tpl" label="ORDER NO" field="seq"}
   {include file="{$sys.uix.elm}/tbl/th_col.tpl" label="IS FOLLOW UP" field="seq"}
{/block}
{block name="td_ext"}
   <td>{$i.seq}</td>
   <td>{$i.is_follow_up}</td>
{/block}