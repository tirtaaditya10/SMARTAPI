...
{/block}
{block name="th_ext"}
    {include file="{$sys.uix.elm}/tbl/th_col.tpl" label="ENTITY" field="upl_entity_type" break_point="all"}
...
{block name="td_ext"}
    ...
    <td name="stage" data-val="{$i.upl_stage_id}">{$i.upl_stage}</td>
    <td name="stats" data-val="{$i.upl_status_id}">{$i.upl_status}</td>
...
{block name="jquery_page_ready"}
    $('table.footable tbody tr').click(function(e) {
        ...
        var row     = $(this),
            stage   = parseInt(row.find('td[name="stage"]').data('val')),
            stats   = parseInt(row.find('td[name="stats"]').data('val'));
...
{block name="script_page_pst"}
    <script>
        ...
        function ExportToTable() {
            ...
            if (regex.test($("#svc_file").val().toLowerCase())) {
	            // Flag for checking whether excel is .xls format or .xlsx format
                var flag_xlsx = false;
                var file_name = $("#svc_file").val();
                ...
                if (typeof (FileReader) != "undefined") {
                    ...
                    if (flag_xlsx) {
                        /*If excel file is .xlsx extension than creates a Array Buffer from excel*/
                         reader.readAsArrayBuffer($("#svc_file")[0].files[0]);
                     }
                    else {
                        reader.readAsBinaryString($("#svc_file")[0].files[0]);
                    }
                }
                else {
...
{/block}
{block name="script_external"}
    <script src="{$sys.cfg.asset}js/plugin/sheet.js/js/xlsx.full.min.js"></script>
{/block}
