...
<span class="fileinput-button">
	...
	<input type="file" name="file" id="uf_icon"
		   data-url="{$sys.req.rid}/{$obj|default:'doc'}/{$sys.req.pid}"
		   data-loc="{$loc|default:'fix'}"
		   data-cat="{$dms_catalog_id}"
		   data-idc="{$sys.rsp.dat.id_code}"
...
$(function () {
	...
	*/
	$('#uf_icon').change(function(e) {
		var that = $(this),
			dirx = that.data('url');
		if(that.attr('id') == 'uf_icon') {
			var data = new FormData(),
				file = upload_file_util($('#uf_icon').val()),
				dFrm = $('form.cForm'),
				dBdy = $('#uplBody'),
				dBar = '',
				dTR  = '';
			...
			if(e && e.target && e.target.files && e.target.files[0]) {
				...
				if($('#dms_catalog_id_{$sys.req.rid}', dFrm).length)
					data.append('dms_catalog_id', $('#dms_catalog_id_{$sys.req.rid}', dFrm).val());
				if($('#dms_publisher_id_{$sys.req.rid}', dFrm).length)
					data.append('dms_publisher_id', $('#dms_publisher_id_{$sys.req.rid}', dFrm).val());
				if($('#crm_company_id_{$sys.req.rid}', dFrm).length)
					data.append('crm_company_id', $('#crm_company_id_{$sys.req.rid}', dFrm).val());
				...
				$.ajax({
					...
					success: function(data, textStatus, jqXHR) {
						...
						if(rst.ack) {
							...
							obj.data('url', obj.data('url').replace('##new_id', rst.dat.id));
							obj.attr('data-url', obj.data('url').replace('##new_id', rst.dat.id));
