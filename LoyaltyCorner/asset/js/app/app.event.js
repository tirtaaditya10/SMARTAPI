var bs3ui = function () {
	...
	var handle_datetimepicker = function () {
		if (jQuery().datetimepicker) {
			$("div.datetime_picker").datetimepicker({
	...
	var handle_selectpicker = function () {
		if ($('select.selectpicker').length) {
			if ($().selectpicker)
				...
				load_asset('js/plugin/bootstrap-select/css/bootstrap-select.min.css', function () {
					load_asset('js/plugin/bootstrap-select/js/bootstrap-select.min.js', handle_selectpicker);
				});
			}
		}
	};
	...
	var handle_select2 = function() {
		if($('.select2').length || $('.select2-multiple').length || $('.select2tags').length) {
			if ($().select2) {
				...
				loadScript(AGV.asset + 'js/plugin/select2/select2.min.js', handle_select2);
			}
		}
		/*
			if ($().select2) {
	...
	var handle_editor = function () {
		if (jQuery().wysihtml5)
			$('.wysihtml5').wysihtml5({
				"stylesheets": [AGV.asset + 'js/plugin/bootstrap-wysihtml5/wysiwyg-color.css']
			});
	...
	var handle_footable_grid = function() {
		if ($('#PCL table.footable').length) {
			if ($().footable)
				...
				loadScript(AGV.asset + 'js/plugin/moment/moment.min.js', function () {
					load_asset('js/plugin/footable/css/footable.bootstrap.min.css');
					loadScript(AGV.asset +'js/plugin/footable/js/footable.min.js', handle_footable_grid);
				});
			}
		}
	};
	var handle_footable_form = function() {
		if ($('#PCD table.footable').length) {
			if ($().footable)
				...
				loadScript(AGV.asset + 'js/plugin/moment/moment.min.js', function () {
					load_asset('js/plugin/footable/css/footable.bootstrap.min.css');
					loadScript(AGV.asset +'js/plugin/footable/js/footable.min.js', handle_footable_form);
				});
			}
		}
	};
	...
	var handle_treeGrid_grid = function() {
		if($('#PCL table.treeGrid').length) {
			if ($().treegrid) {
				...
				load_asset('js/plugin/jquery-treegrid/css/jquery.treegrid.css', function () {
					load_asset('js/plugin/jquery-treegrid/js/jquery.cookie.js', function () {
						load_asset('js/plugin/jquery-treegrid/js/jquery.treegrid.js', function () {
							load_asset('js/plugin/jquery-treegrid/js/jquery.treegrid.bootstrap3.js', handle_treeGrid_grid);
						});
					});
				});
			}
		...
		if($('#PCD table.treeGrid').length) {
			if ($().treegrid) {
				...
				load_asset('js/plugin/jquery-treegrid/css/jquery.treegrid.css', function () {
					load_asset('js/plugin/jquery-treegrid/js/jquery.cookie.js', function () {
						load_asset('js/plugin/jquery-treegrid/js/jquery.treegrid.js', function () {
							load_asset('js/plugin/jquery-treegrid/js/jquery.treegrid.bootstrap3.js', handle_treeGrid_form);
						});
					});
				});
			}
	...
	var handle_audio = function () {
		if ($('audio').length) {
			if ($().maudio) {
				...
				load_asset('js/plugin/maudio/css/maudio.css', function () {
					load_asset('js/plugin/maudio/js/maudio.js', handle_audio);
				});
			}
		}
	}
	...
	var handle_cForm = function() {
		// for more info visit the official plugin documentation:
		// http://docs.jquery.com/Plugins/Validation
		$('form.cForm').each(function() {
			$(this).cFormValidate();
		});
	};
...
$.fn.cFormValidate = function(event) {
	AGV.form   = $(this);
	var dForm  = $(this);
	...
	if ($().datepicker) {
		$('.date-picker .form-control', dForm).change(function () {
			dForm.validate().element($(this)); //revalidate the chosen dropdown value and show error or success message for the input
		})
	}
}
...
jQuery(function() {
	...
	$content.on('submit', 'form.sForm', function(e) {
		e.preventDefault();
		e.stopPropagation();
		
		var $this   = $(this);
		AGV.target  = $($(this).data('target') ? $(this).data('target') : '#PCT');
